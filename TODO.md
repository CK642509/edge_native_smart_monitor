...
## 步驟八：測試與驗證
- [ ] 新增測試基礎：為 `CameraStream` 假影像、`RingBuffer`、`VideoRecorder` 建立 fixture 與單元測試，確保在沒有硬體時也能完整驗證。
- [ ] API/整合測試：模擬監控啟停、錄影觸發、串流端點與設定調整流程，確保 FastAPI 層與核心迴圈協同運作。

## 步驟九：智慧偵測與觸發
- [ ] 簡易偵測器：實作 `PresenceDetector`（如背景減除/YOLO 人型偵測），檢測畫面是否有人，並將結果以事件形式送回 `MonitorSystem`。
- [ ] 偵測驅動錄影：當畫面有人時暫停錄影需求，一旦連續 N 幀無人，將前後 10 秒畫面從緩衝區寫入檔案，並加入冷卻/去抖動設定。
